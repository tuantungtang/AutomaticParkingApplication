...
from TensorFlow.TVT.connectToMysql import connect


class CameraThread(QtCore.QThread):
    change_pixmap_signal_cam1 = QtCore.pyqtSignal(QtGui.QImage)
    change_pixmap_signal_cam2 = QtCore.pyqtSignal(QtGui.QImage)
    # khai báo biến gửi dữ liệu lên giao diện
    result_signal = QtCore.pyqtSignal(str)  # Add this line
    ...
    def run(self):
        ...
        while True:
            ...
            # Decode mã vạch trong frame
                decoded_objects = pyzbar.decode(frame1)
            except:
                print("loi doc du lieu")
            ...
            for obj in decoded_objects:
                ...
                # print(result_text)
                # Emit the result signal with the obtained result_text
                # Phần của Nhi==================
                resultMV = connect(obj.data.decode('utf-8'))
                # khởi tạo biến tạm loại xe
                type_read = 'ab'
                # khởi tạo biến tạm biển số xe
                license_read = "99H77060"
                print(resultMV)
                # nen ket qua tra ve
                resultMVCV = "" + resultMV[0] + "," + resultMV[1] + "," + resultMV[2]

                if license_read == resultMV[2]:
                    resultMVCV = resultMVCV + "," + "true"
                else:
                    resultMVCV = resultMVCV + "," + "false"
                    # gui ket qua tra ve
                self.result_signal.emit(resultMVCV)
...
class Ui_MainWindow(object):
    ...
    def retranslateUi(self, MainWindow):
        ...
        # self.ID.setTextElideMode("MainWindow",)
